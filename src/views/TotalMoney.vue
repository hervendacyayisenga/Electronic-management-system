...
function generatePDF() {
  ...
  doc.setFontSize(11)
  doc.text(`Accumulated Assets (Sold): ${rwf(subtotal.value)}`, 14, 48)
  doc.text(`Projected Pipeline (Pending): ${rwf(totalPending.value)}`, 14, 55)
  ...
  if (store.successfulProducts.length > 0) {
    ...
    doc.text('Successful Transactions', 14, startY)
    
    doc.autoTable({
      startY: startY + 5,
      head: [['ID', 'Name', 'Price', 'Qty', 'Subtotal']],
  ...
  if (store.pendingProducts.length > 0) {
    ...
    doc.text('Pending Inventory', 14, startY)
    
    doc.autoTable({
      startY: startY + 5,
      head: [['ID', 'Name', 'Price', 'Qty', 'Potential Value']],
