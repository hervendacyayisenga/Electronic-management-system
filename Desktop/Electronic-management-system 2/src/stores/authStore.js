...
export const useAuthStore = defineStore('auth', {
    ...
    actions: {
        ...
        login(email, password) {
            ...
            // Success – reset failed attempts and indicate OTP is required
        ...
        // Admin reset password for any user (called from AccountsView)
        adminResetPassword(email, newPassword) {
            if (newPassword.length < 10 || !/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9])/.test(newPassword)) {
                return { ok: false, error: 'Password must be at least 10 characters and include uppercase, lowercase, a number, and a special character.' }
            }
            const users = getUsers()
        ...
        // Reset password directly (bypassing token flow)
        directPasswordReset(email, newPassword) {
            this.clearMessages()
            if (newPassword.length < 10 || !/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9])/.test(newPassword)) {
                this.loginError = 'Password must be at least 10 characters and include a number, uppercase, lowercase, and special character.'
                return false
            }
        ...
        generateLoginOTP(email) {
            ...
            const otps = JSON.parse(localStorage.getItem('ems_login_otps') || '{}')
            // OTP expires in 5 minutes
            otps[email.toLowerCase()] = { code: otpCode, expires: Date.now() + 5 * 60000 }
            localStorage.setItem('ems_login_otps', JSON.stringify(otps))
            
            return otpCode
        },

        // Verify the given OTP for the given email
        verifyLoginOTP(email, otpStr) {
            this.clearMessages()
            const otps = JSON.parse(localStorage.getItem('ems_login_otps') || '{}')
            const emailKey = email.toLowerCase()
            const otpData = otps[emailKey]

            if (!otpData) {
                this.loginError = 'No OTP requested or it has expired.'
                return false
            }
            ...
            if (Date.now() > otpData.expires) {
                ...
                localStorage.setItem('ems_login_otps', JSON.stringify(otps))
                this.loginError = 'This OTP has expired. Please request a new one.'
                return false
            }

            if (otpData.code !== otpStr) {
