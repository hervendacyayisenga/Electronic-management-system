...
const forgotStep = ref(1) // 1: Waiting for Email, 2: Setting New Password
const forgotNewPassword = ref('')
const forgotConfirmPassword = ref('')
const showForgotNewPass = ref(false)
const showForgotConfirmPass = ref(false)
...
async function handleLogin() {
  ...
  if (result && result.success) {
    if (result.requireOTP) {
      pendingLoginEmail.value = loginEmail.value
      pendingOTP.value = authStore.generateLoginOTP(loginEmail.value)
...
async function handleVerifyOTP() {
  ...
  const success = authStore.verifyLoginOTP(pendingLoginEmail.value, otpCode.value.trim())
...
function resendOTP() {
  pendingOTP.value = authStore.generateLoginOTP(pendingLoginEmail.value)
}

// Auto-fill and login as Super Admin
async function loginAsAdmin() {
  loginEmail.value = 'ndacyayisengaherve8@gmail.com'
...
// Handle forgot password submission steps
async function handleForgotSubmit() {
  ...
  if (forgotStep.value === 1) {
    ...
    if (!user) {
      ...
      forgotStep.value = 2 // Move to new password step
    }
    ...
    if (forgotNewPassword.value !== forgotConfirmPassword.value) {
      ...
      const ok = authStore.directPasswordReset(forgotEmail.value, forgotNewPassword.value)
      if (ok) {
        switchTab('login')
        ...
        authStore.registerSuccess = 'Password reset successfully! You can now sign in.'
      }
    }
  }
  ...
  <div class="min-h-screen flex relative bg-[#0B0F19]">
    ...
    <div class="relative z-10 flex-1 flex flex-col justify-center items-center px-4 sm:px-6 lg:px-8 py-12 w-full">
      ...
      <div class="w-full max-w-md bg-white/10 backdrop-blur-xl rounded-2xl shadow-2xl overflow-hidden border border-white/10 ring-1 ring-white/5">
        ...
        <!-- Tab switcher (Hidden in forgot and otp mode) -->
        <div v-if="activeTab !== 'forgot' && activeTab !== 'otp'" class="flex border-b border-white/10 mx-6 mt-6">
          <button
        ...
        <form v-else-if="activeTab === 'forgot'" aria-label="Forgot Password Form" @submit.prevent="handleForgotSubmit" class="px-6 py-6 space-y-5">
          ...
          <div v-else class="space-y-4">
            <p class="text-sm text-slate-400 mb-6">Set a new password for <strong class="text-white">{{ forgotEmail }}</strong></p>
            
            <div>
              <label for="forgotNewPassword" class="sr-only">New Password (min 10 characters)</label>
              <div class="relative group">
                <span class="absolute inset-y-0 left-3 flex items-center text-slate-400 group-focus-within:text-cyan-400 transition-colors" aria-hidden="true">
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                ...
                <input id="forgotNewPassword" v-model="forgotNewPassword" :type="showForgotNewPass ? 'text' : 'password'" placeholder="New Password (min 10 characters)" required
                  class="w-full pl-9 pr-10 py-3 bg-[#0B0F19]/60 border border-white/10 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all"/>
                <button type="button" @click="showForgotNewPass = !showForgotNewPass" :aria-label="showForgotNewPass ? 'Hide password' : 'Show password'" class="absolute inset-y-0 right-3 flex items-center text-slate-400 hover:text-white focus:outline-none focus:text-cyan-400 focus:ring-2 focus:ring-cyan-500 rounded-md transition-colors">
                  <svg v-if="!showForgotNewPass" class="w-4 h-4" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                  </svg>
                  <svg v-else class="w-4 h-4" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-5.523 0-10-4.477-10-8 0-1.09.252-2.124.7-3.05M6.34 6.34A9.956 9.956 0 0112 5c5.523 0 10 4.477 10 8 0 1.347-.328 2.617-.912 3.737M3 3l18 18"/>
            ...
            <div>
              <label for="forgotConfirmPassword" class="sr-only">Confirm new password</label>
              <div class="relative group">
                <span class="absolute inset-y-0 left-3 flex items-center text-slate-400 group-focus-within:text-cyan-400 transition-colors" aria-hidden="true">
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/>
                ...
                <input id="forgotConfirmPassword" v-model="forgotConfirmPassword" :type="showForgotConfirmPass ? 'text' : 'password'" placeholder="Confirm new password" required
                  class="w-full pl-9 pr-10 py-3 bg-[#0B0F19]/60 border border-white/10 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all"/>
                <button type="button" @click="showForgotConfirmPass = !showForgotConfirmPass" :aria-label="showForgotConfirmPass ? 'Hide confirm password' : 'Show confirm password'" class="absolute inset-y-0 right-3 flex items-center text-slate-400 hover:text-white focus:outline-none focus:text-cyan-400 focus:ring-2 focus:ring-cyan-500 rounded-md transition-colors">
                  <svg v-if="!showForgotConfirmPass" class="w-4 h-4" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                  </svg>
                  <svg v-else class="w-4 h-4" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-5.523 0-10-4.477-10-8 0-1.09.252-2.124.7-3.05M6.34 6.34A9.956 9.956 0 0112 5c5.523 0 10 4.477 10 8 0 1.347-.328 2.617-.912 3.737M3 3l18 18"/>
            ...
            <button @click="forgotStep = 1" type="button" class="mt-4 w-full py-3 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 font-bold text-sm transition-all focus:outline-none focus:ring-2 focus:ring-slate-500">
              Back to Verification
            </button>
          </div>

          <div aria-live="polite" class="min-h-[20px]" v-if="authStore.loginError">
            <p class="text-red-400 text-xs px-1" role="alert">{{ authStore.loginError }}</p>
          </div>

          <button type="submit" :disabled="forgotLoading"
            :aria-busy="forgotLoading"
            class="w-full py-3 rounded-lg bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-bold text-sm transition-all shadow-[0_0_15px_rgba(6,182,212,0.4)] hover:shadow-[0_0_25px_rgba(6,182,212,0.6)] disabled:opacity-50 disabled:shadow-none focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:ring-offset-2 focus:ring-offset-[#0B0F19]">
            {{ forgotLoading ? 'Processing...' : (forgotStep === 1 ? 'Verify Email' : 'Reset Password') }}
          </button>
        </form>

        <!-- OTP VERIFICATION FORM -->
        <form v-else-if="activeTab === 'otp'" aria-label="OTP Verification Form" @submit.prevent="handleVerifyOTP" class="px-6 py-8 space-y-6">
          <div class="text-center">
            <h3 class="text-xl font-bold text-white mb-2">Two-Step Verification</h3>
            <p class="text-sm text-slate-400">For this demo, your 6-digit verification code is: <span class="font-bold text-emerald-400 bg-emerald-900/40 px-2 py-1 rounded ml-1 tracking-wider border border-emerald-500/30">{{ pendingOTP }}</span></p>
          </div>

          <div>
            <label for="otpCode" class="sr-only">6-Digit Code</label>
            <div class="relative group">
              <span class="absolute inset-y-0 left-3 flex items-center text-slate-400 group-focus-within:text-cyan-400 transition-colors" aria-hidden="true">
              ...
              <input id="otpCode" v-model="otpCode" type="text" maxlength="6" placeholder="Enter 6-digit code" required
                class="w-full pl-10 pr-4 py-3 bg-[#0B0F19]/60 border border-white/10 rounded-lg text-lg text-center tracking-widest font-mono text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all"/>
            </div>
            <div aria-live="polite" class="min-h-[20px] mt-2 text-center">
              <p v-if="authStore.loginError" class="text-red-400 text-xs px-1" role="alert">{{ authStore.loginError }}</p>
            </div>
          </div>
          ...
          <button type="submit" :disabled="otpLoading || otpCode.length !== 6"
            class="w-full flex items-center justify-center py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg text-base font-bold transition-all shadow-[0_0_15px_rgba(8,145,178,0.4)] disabled:opacity-50 disabled:shadow-none focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 focus:ring-offset-[#0B0F19]">
            <svg v-if="otpLoading" class="animate-spin -ml-1 mr-2 w-5 h-5 text-white" fill="none" viewBox="0 0 24 24">
              <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
              <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            Verify Code
          ...
          <div class="flex items-center justify-between text-sm mt-4">
            <button @click="switchTab('login')" type="button" class="text-slate-400 hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-cyan-500 rounded px-1 flex items-center gap-1">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
              Back to login
            </button>
            <button type="button" @click="resendOTP" class="text-cyan-400 hover:text-cyan-300 transition-colors focus:outline-none focus:ring-2 focus:ring-cyan-500 rounded px-1">
              Resend Code
            </button>
          </div>
        </form>
